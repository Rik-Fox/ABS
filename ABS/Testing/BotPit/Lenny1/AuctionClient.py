...
class AuctionClient(object):
    ...
    def __init__(self, host="localhost", port=8020, mybidderid=None, verbose=False):
        ...
        self.points = {name: 0 for name in self.players}
    ...
    def third_bidding_strategy(self, numberbidders, wincondition, artists, values, rd, itemsinauction, winnerarray, winneramount, mybidderid, players, standings, winnerpays):
        ...
        for i in itemsinauction[rd:]:
            remainingPoints+=values[i]

        for player in players:
            totalStandings += standings[player]['money']

        if rd>0:
            self.points[winnerarray[rd-1]]+=values[itemsinauction[rd-1]]

        myValues = {artist: numberbidders*values[artist]*standings[mybidderid]['money']/remainingPoints for artist in artists }
        return int((myValues[itemsinauction[rd]]+self.random_bid(standings[mybidderid]['money']))/2)
        #return int(self.random_bid(standings[mybidderid]['money']))
        #return 40
