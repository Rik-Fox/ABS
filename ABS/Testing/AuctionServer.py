...
class AuctionServer(object):
    ...
    def announce_auction(self):
        listtosend = str(self.numbidders) + ' ' + str(len(self.itemtypes)) + ' ' + str(self.auction_size) + ' ' + str(
            self.budget) + ' ' + str(self.neededtowin) + ' ' + str(self.announce_order) + ' ' + str(self.winner_pays) + ' '
        ...
        for conn in self.conns:
            ...
            for s in socks:
                ...
                if indata[0] in self.bidderids:
                    print(
                        "Already have a bidder called %s, please ensure all biddierids are unique!" % indata[0])
                    raise ValueError
                # print s.getpeername()
        ...
        for conn in self.conns:
            ...
            for s in socks:
                data = s.recv(1024).decode('utf_8')
                if data != 'connected ':
                    print("Did not receive connect signal from %s" %
                          self.porttobidders[s.getpeername()])
                    raise IOError
        ...
        for conn in self.conns:
            ...
            for s in socks:
                data = s.recv(1024).decode('utf_8')
                if data != 'ready ':
                    print("Did not receive ready signal from %s" %
                          self.porttobidders[s.getpeername()])
                    raise IOError
    ...
    def run_auction(self):
        ...
        while not done:
            ...
            append_to_csv([str(payment/self.values[currentitem])], "money_per_point.csv")
            ...
            if self.neededtowin > 0:
                if self.standings[winner][currentitem] >= self.neededtowin:
                    won = winner
                    print("%s has won with %d %s!" %
                          (winner, self.standings[winner][currentitem], currentitem))
                    done = True
            for name in self.bidderids:
                if not done:
        ...
        ###############################################
        # CUSTOM CODE FOR BOT ARENA ADDED BELOW ONLY
        ###############################################
        # import datetime
        #
        # points = self.standings[won]['Picasso']*self.values['Picasso'] + self.standings[won]['Da_Vinci']*self.values['Da_Vinci'] + \
        #     self.standings[won]['Rembrandt']*self.values['Rembrandt'] + \
        #     self.standings[won]['Van_Gogh']*self.values['Van_Gogh']
        # my_points = self.standings['A0Policy']['Picasso']*self.values['Picasso'] + self.standings['A0Policy']['Da_Vinci']*self.values['Da_Vinci'] + \
        #     self.standings['A0Policy']['Rembrandt']*self.values['Rembrandt'] + \
        #     self.standings['A0Policy']['Van_Gogh']*self.values['Van_Gogh']
        #
        # paintings = [self.standings[won]['Picasso'], self.standings[won]['Van_Gogh'],
        #              self.standings[won]['Rembrandt'], self.standings[won]['Da_Vinci']]
        # my_paintings = [self.standings['A0Policy']['Picasso'],  self.standings['A0Policy']['Van_Gogh'],
        #                 self.standings['A0Policy']['Rembrandt'], self.standings['A0Policy']['Da_Vinci']]
        #
        # csv_list = [str(datetime.datetime.now()), self.numbidders,
        #             won, self.standings[won]['money'], points, paintings]
        # my_list = [str(datetime.datetime.now()), self.numbidders,
        #            'A0Policy', self.standings['A0Policy']['money'], my_points, my_paintings]
        #
        # output_filename = "RL_data/exported_results.csv"
        # me = "RL_data/individual_results.csv"
        # append_to_csv(csv_list, output_filename)
        # append_to_csv(my_list, me)

    def get_standings(self):
        # Maybe show standings here
        # 1. Games 1 and 2 - first to five
        standings_list = []
        if self.neededtowin != 0:
            # Mark what game number it is
            if self.announce_order == True:
                standings_list.append("Game 1")
            elif self.announce_order == False:
                standings_list.append("Game 2")
            else:
                standings_list.append("Unknown Game")

            for name in self.standings:

                this_players_paintings = self.standings[name].copy()
                del this_players_paintings['money']
                this_players_max = max(this_players_paintings, key=this_players_paintings.get)

                print(name, "\t", this_players_paintings[this_players_max], this_players_max)
                standings_list += [name, this_players_paintings[this_players_max], this_players_max]
            print("\n")

        # Games 3 and 4 - total value
        elif self.neededtowin == 0:
            # Mark what game number it is
            if self.winner_pays == 0:
                standings_list.append("Game 3")
            elif self.winner_pays == 1:
                standings_list.append("Game 4")
            else:
                standings_list.append("Unknown Game")

            # Add rankings and value of apintings to csv list.
            ...
            for player in sorted_values:
                standings_list.append(player[0])
                standings_list.append(player[1])
