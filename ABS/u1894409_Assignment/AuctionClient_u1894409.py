...
class AuctionClient_u1894409(object):
    ...
    def multiplier(self, winnerarray, mybidderid):
        if mybidderid in winnerarray and winnerarray[-1] == mybidderid:

            self.loss = math.floor(self.loss/2)
        else:
            self.loss += 1
        ...
        mult = 1
        for i in range(0, self.loss):
            mult *= 1.25

        return mult

    def valuation(self, whats_left, values, budget):
        ...
        if points_left == 0:
            ...
            value = unit_value*values[whats_left[0]]
            return math.ceil(value)

    def freq(self, first, artists, known):

        if known is True:
            arts = {'Picasso': first.count('Picasso'),
            ...
            return sorted(arts.items(), key=lambda x: x[1])[1][0]

        else:

            return sorted(artists.items(), key=lambda item: (item[1], item[0]))[1][0]
    ...
    def first_bidding_strategy(self, numberbidders, wincondition, artists, values, rd, itemsinauction, winnerarray, winneramount, mybidderid, players, standings, winnerpays):
        ...
        expected_rounds = int(math.ceil(5*numberbidders))
        if expected_rounds > 200:
            expected_rounds = 200

        pick = self.freq(itemsinauction[0:expected_rounds], artists, known=True)

        # print(pick)
        # number_won = len(list(filter(lambda x: x == "u1894409", winnerarray)))
        # print(number_won)

        if itemsinauction[rd] == pick:
            return 200
        else:
            return 0
    ...
    def second_bidding_strategy(self, numberbidders, wincondition, artists, values, rd, itemsinauction, winnerarray, winneramount, mybidderid, players, standings, winnerpays):
        ...
        pick = self.freq(itemsinauction, artists, known=False)
    ...
    def third_bidding_strategy(self, numberbidders, wincondition, artists, values, rd, itemsinauction, winnerarray, winneramount, mybidderid, players, standings, winnerpays):
        ...
        value = self.valuation(itemsinauction[rd:-1],
                               values, standings[mybidderid]['money'])

        util = ((numberbidders-1)/numberbidders)*math.sqrt(numberbidders) * \
            value*self.multiplier(winnerarray, mybidderid)
    ...
    def fourth_bidding_strategy(self, numberbidders, wincondition, artists, values, rd, itemsinauction, winnerarray, winneramount, mybidderid, players, standings, winnerpays):
        ...
        util = value*math.sqrt(numberbidders)*self.multiplier(winnerarray, mybidderid)
