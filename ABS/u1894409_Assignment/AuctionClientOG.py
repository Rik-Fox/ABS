...
class AuctionClientOG(object):
    ...
    def random_bid(self, budget):
        ...
        return int(50*random.random()+1)

    def valuation(self, whats_left, current, budget):
        ...
        if points_left == 0:
            ...
            Return = budget/points_left

            if current == 'Picasso':
                Return *= 4
            if current == 'Van_Gogh':
                Return *= 6
            if current == 'Rembrandt':
                Return *= 8
            if current == 'Da_Vinci':
                Return *= 12
    ...
    def freq(self, itemsinauction, artists, known):

        if known == True:
            first = itemsinauction[0:70]
    ...
    def first_bidding_strategy(self, numberbidders, wincondition, artists, values, rd, itemsinauction, winnerarray, winneramount, mybidderid, players, standings, winnerpays):
        ...
        pick = self.freq(itemsinauction, artists, known=True)
    ...
    def third_bidding_strategy(self, numberbidders, wincondition, artists, values, rd, itemsinauction, winnerarray, winneramount, mybidderid, players, standings, winnerpays):
        ...
        value = self.valuation(itemsinauction[rd:-1],
                               itemsinauction[rd], standings[mybidderid]['money'])
        util = ((numberbidders-1)/numberbidders)*math.sqrt(numberbidders)*value
        return int(util)
